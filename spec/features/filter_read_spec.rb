...
RSpec.describe 'can filter read links', js: true do
  scenario 'filter to see only read links' do
    user = create(:user)
    ...
    expect(page).to have_text('http://firsturl.com')
    expect(page).to_not have_text('Second Title')
    expect(page).to_not have_text('http://secondurl.com')
  end
end
